...
public class PHDLNetList extends HashMap<PHDLNet, HashSet<PHDLPin>> {
	...
	/**
	 * Returns a set of pins associated with a specific
	 * net.
	 * 
	 * Netlist pins accessor method
	 * 
	 * @param net the net associated with the pins
	 ...
	 * 
	 * @param net the net to remove from the netlist
	 * @return	true if the net was in the netlist
	 * 			false, otherwise
	 */
	 ...
	 * @return	true if the pin is in the netlist
	 * 			false, otherwise
	 */
	 ...
	 * @return	true if the net is in the netlist
	 * 			false, otherwise
	 */
	 ...
	 * 
	 * @param net the net to check
	 * @return	true if the net has pins
	 * 			false if the net doesn't have pins
	 * 			false if the net isn't in the netlist
	 */
	...
	public static boolean unitTest() {
		...
		if (netlist.getNumberPins(net1) != 1) {
			success = false;
			TestDriver.err("getNumberNets()", "1", "" + netlist.getNumberPins(net1));
		}
		...
		if (!netlist.containsNet(net1)) {
			success = false;
			TestDriver.err("containsNet()", "to have \"net1\"", "did not have it");
		}
		
		if (!netlist.containsPin(new PHDLPin("net1pin1", 1))) {
			success = false;
			TestDriver.err("containsPin()", "to have \"net1pin1\"", "did not have it");
		}
		if (!netlist.containsPin(new PHDLPin("net1pin2", 2))) {
			success = false;
			TestDriver.err("containsPin()", "to have \"net1pin2\"", "did not have it");
		}
		
		if (netlist.getNumberNets() != 1) {
			success = false;
			TestDriver.err("getNumberNets()", "1", "" + netlist.getNumberNets());
		}
		
		if (netlist.getNumberPins(net1) != 2) {
			success = false;
			TestDriver.err("getNumberNets()", "2", "" + netlist.getNumberPins(net1));
		}
		...
		if (myPins.isEmpty()) {
			success = false;
			TestDriver.err("getPins()", "a non-empty HashSet", "an empty HashSet");
		}
		if (!myPins.contains(new PHDLPin("net1pin1", 1))) {
			success = false;
			TestDriver.err("getPins()", "to have \"net1pin1\"", " didn't have it");
		}
		if (!myPins.contains(new PHDLPin("net1pin2", 2))) {
			success = false;
			TestDriver.err("getPins()", "to have \"net1pin2\"", " didn't have it");
		}
		if (myPins.size() != 2) {
			success = false;
			TestDriver.err("getPins()", "to have only 2 elements", "has " + myPins.size() + " elements");
		}
		...
		if (newNetList.getNumberNets() != 1) {
			success = false;
			TestDriver.err("addPin()", "number of nets to be 1", "was" + newNetList.getNumberNets());
		}
		if (newNetList.getNumberPins(net2) != 1) {
			success = false;
			TestDriver.err("addPin()", "number of pins for \"net2\" to be 1", "was" + newNetList.getNumberPins(net2));
		}
		...
		if (newNetList.getNumberNets() != 1) {
			success = false;
			TestDriver.err("addPin()", "number of nets to be 1 after second addition", "was" + newNetList.getNumberNets());
		}
		if (newNetList.getNumberPins(net2) != 2) {
			success = false;
			TestDriver.err("addPin()", "number of pins for \"net2\" to be 2", "was" + newNetList.getNumberPins(net2));
		}
		
		newNetList.removePin(net2, pin21);
		if (newNetList.containsPin(pin21)) {
			success = false;
			TestDriver.err("removePin()", "\"pin21\" should have been removed", "it wasn't");
		}
		if (newNetList.getNumberPins(net2) != 1) {
			success = false;
			TestDriver.err("removePin()", "should have 1 pin", "had " + newNetList.getNumberPins(net2));
		}
		if (newNetList.isNetEmpty(net2)) {
			success = false;
			TestDriver.err("removePin()", "should not be an empty net", "empty net");
		}
		
		newNetList.removePin(net2, pin22);
		if (newNetList.containsPin(pin22)) {
			success = false;
			TestDriver.err("removePin()", "\"pin22\" should have been removed", "it wasn't");
		}
		if (newNetList.getNumberPins(net2) != 0) {
			success = false;
			TestDriver.err("removePin()", "should have 0 pins", "had " + newNetList.getNumberPins(net2));
		}
		if (!newNetList.isNetEmpty(net2)) {
			success = false;
			TestDriver.err("removePin()", "should have an empty net", "non-empty");
		}
		...
		if (newNetList.containsNet(net2)) {
			success = false;
			TestDriver.err("removeNet()", "\"net2\" should not be present", "it still is");
		}
		if (newNetList.containsPin(pin21) || newNetList.containsPin(pin22)) {
			success = false;
			TestDriver.err("removeNet()", "should not contain any pins", "does");
		}
		...
		if (newNetList.getNumberPins(net2) != -1) {
			success = false;
			TestDriver.err("removeNet()", "should say there are -1 pins", "says there are " + newNetList.getNumberPins(net2));
		}
		if (!newNetList.isEmpty()) {
			success = false;
		...
		if (!newNetList.containsNet(net1)) {
			success = false;
			TestDriver.err("addAllPins()", "should contain \"net1\"", "doesn't");
		}
		if (!newNetList.containsNet(net2)) {
			success = false;
			TestDriver.err("addAllPins()", "should contain \"net2\"", "doesn't");
		}
		if (!newNetList.containsNet(net3)) {
			success = false;
			TestDriver.err("addAllPins()", "should contain \"net3\"", "doesn't");
		}
		if (newNetList.getNumberNets() != 3) {
			success = false;
			TestDriver.err("addAllPins()", "should have 3 nets", "has " + newNetList.getNumberNets());
		}
		...
		if (newNetList.getNumberPins(net1) != 2) {
			success = false;
			TestDriver.err("addAllPins()", "should have 2 pins in \"net1\"", "has " + newNetList.getNumberPins(net1));
		}
		if (newNetList.getNumberPins(net2) != 2) {
			success = false;
			TestDriver.err("addAllPins()", "should have 2 pins in \"net2\"", "has " + newNetList.getNumberPins(net2));
		}
