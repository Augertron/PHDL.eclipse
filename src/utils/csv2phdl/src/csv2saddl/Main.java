...
/**
 * This Comparator class is used to compare vector pins that have numeric indexes
 * inclosed in <> brackets. In other words std_logic_vectors.
 */
...
/**
 * This program reads Xilinx Comma Separated Value, csv, files and generates
 * customized component declaration and instantiation templates for use in
 * printed circuit HDL designs.
 * 
 * @author pedro
 */
public class Main {
    ...
    /**
     * This procedure prints the component declaration.
     * Use an include statement to use this file in your design.
     *
     * @param SigHash
     * @param PowHash
     * @param UnusedHash 
    ...
    public static void PrintDeclaration(TreeMap SigHash, TreeMap PowHash, TreeMap UnusedHash) {
        ...
        while(it.hasNext()){
            ...
            while(innerIt.hasNext()){
                ...
                System.out.print(((String[])((TreeMap)SigHash.get(KeyString)).get(innerKeyString))[0]);
    ...
    /**
     * This procedure prints out a component instantiation template for the convenience 
     * of the PHDL board designer. Paste the instantiation template into your PHDL code 
     * and wire it up by editing the actuals.
     * 
     * @param SigHash This is the hash table of hash tables that holds the signal pins derived from the FPGA design.
     * @param PowHash This is the hash table of hash tables that holds the power and dedicated pins of the FPGA.
     * @param UnusedHash This is the hash table of hash tables that holds the unused I/O pins of the FPGA.
     */
     ...
     * Columns 0 through 15 are defined as follows.
     * Pin Number,Signal Name, Pin Usage,Pin Name,Direction,IO Standard,IO Bank Number,Drive (mA),Slew Rate,Termination,IOB Delay,Voltage,Constraint,IO Register,Signal Integrity
     * 
     * Then let's find all the unique signal identifiers.
     * We need a little logic to decide what to do with the signal.
     * If there is a non-null string in column 1 then the signal comes from the compiled FPGA design.
     * Vector signals from the compiled design should be combined into a single HDL component pin.
     * Next we can look for "UNUSED" in column 4. Unused pins could be left off the component definition or
     * just appended to the bottom of the file for convenience. For now we append.
     * Otherwise, we should look to see if it is a power or ground pin by looking in column 3.
     * Power and grounds can be found in column 3 and need to be grouped, ie we want pin [83:0] GND, pin [5:0] VCCO_3, ...
     *
     * This program works by building an informal AST by constructing a hash tables of hash tables.
     * The top hash tables, SigHash and PowHash use the name of the component pin as the key. The value that goes 
     * into SigHash is another hash table that contains the list of physical fpga pins that make up the
     * HDL component pin. The physical pins within a vector pin must be sorted using a special hash sort so that
     * they are in the correct order, ie:  pin [15:0] DQ "AJ7, Y14, K17, ..."
     * 
     * @param args arg[0] is the input filename.
                ...
                if(CSVStringArray[r][1] != ""){ // if this pin has a signal name.
                    ...
                    if (IndexPatMatcher.find()){ // This pin is part of a std_logic_vector.
                        ...
                        if (SigHash.containsKey( RootName )){
                            ((TreeMap) SigHash.get(RootName)).put(CSVStringArray[r][1], CSVStringArray[r]);
                        } else {
                    ...
                    if (!CSVStringArray[r][4].contains("UNUSED")) { // If not an unused pin.
                        ...
                        if (PowHash.containsKey( RootName )){
                            ((TreeMap) PowHash.get(RootName)).put(CSVStringArray[r][0], CSVStringArray[r]);
                        } else {
                            PinHash = new TreeMap();
