...
public class PHDLNetList extends HashMap<PHDLNet, HashSet<PHDLPinVector>> {
	...
	public void addAllPins(PHDLNetList nl) {
		...
		for (PHDLNet a : netSet) {
			if (this.containsKey(a)) {
				...
				this.put(a, new HashSet<PHDLPinVector>());
				this.get(a).addAll(nl.get(a));
			}
		}
	}
	...
	public boolean addPin(PHDLNet net, PHDLPinVector pin) {
		if (this.containsKey(net)) {
			...
			this.put(net, new HashSet<PHDLPinVector>());
			return this.get(net).add(pin);
		}
	}
	 ...
	 */
	public HashSet<PHDLPinVector> getPins(PHDLNet net) {
		return this.get(net);
	}
	 ...
	 */
	public boolean removePin(PHDLNet net, PHDLPinVector pin) {
		HashSet<PHDLPinVector> pinSet = this.get(net);
		if (pinSet != null) {
	 ...
	 */
	public boolean containsPin(PHDLPinVector pin) {
		Collection<HashSet<PHDLPinVector>> pinSets = this.values();
		for (HashSet<PHDLPinVector> s : pinSets) {
			if (s.contains(pin)) {
				return true;
			}
		}
	...
	public String toString() {
		...
		for (PHDLNet n : this.keySet()) {
			HashSet<PHDLPinVector> pins = this.getPins(n);
	...
	// netlist.put(net1, new HashSet<PHDLPinVector>());
	// if (netlist.isEmpty()) {
	// success = false;
	...
	// }
	// netlist.get(net1).add(new PHDLPinVector("net1pin1", 1));
	//
	...
	// }
	// netlist.get(net1).add(new PHDLPinVector("net1pin2", 2));
	//
	...
	// }
	//
	// if (!netlist.containsPin(new PHDLPinVector("net1pin1", 1))) {
	// success = false;
	// TestDriver.err("containsPin()", "to have \"net1pin1\"",
	// "did not have it");
	// }
	// if (!netlist.containsPin(new PHDLPinVector("net1pin2", 2))) {
	// success = false;
	...
	// }
	//
	// HashSet<PHDLPinVector> myPins = netlist.getPins(net1);
	// if (myPins == null) {
	// success = false;
	...
	// }
	// if (!myPins.contains(new PHDLPinVector("net1pin1", 1))) {
	// success = false;
	// TestDriver.err("getPins()", "to have \"net1pin1\"", " didn't have it");
	// }
	// if (!myPins.contains(new PHDLPinVector("net1pin2", 2))) {
	// success = false;
	...
	// PHDLPinVector pin21 = new PHDLPinVector("pin21", 1);
	// PHDLPinVector pin22 = new PHDLPinVector("pin22", 2);
	...
	// success = false;
	// TestDriver.err("addPin()", "number of pins for \"net2\" to be 1", "was" +
	...
	// success = false;
	// TestDriver.err("addPin()", "number of pins for \"net2\" to be 2", "was" +
	// newNetList.getNumberPins(net2));
	// }
	//
	...
	// netlist.addPin(net3, new PHDLPinVector("net3pin5", 5));
	//
	// newNetList.addPin(net2, pin21);
	// newNetList.addPin(net2, pin22);
	// newNetList.addPin(net1, new PHDLPinVector("net1pin2", 2));
	// newNetList.addPin(net1, new PHDLPinVector("net1pin3", 3));
	//
	...
	// }
	//
	// if (!newNetList.containsPin(new PHDLPinVector("net1pin2", 2))) {
	// success = false;
	// TestDriver.err("addAllPins()", "should have \"pin12\"", "doesn't");
	// }
	// if (!newNetList.containsPin(new PHDLPinVector("net1pin3", 3))) {
	// success = false;
	// TestDriver.err("addAllPins()", "should have \"pin13\"", "doesn't");
	// }
	// if (!newNetList.containsPin(new PHDLPinVector("net3pin5", 5))) {
	// success = false;
	...
	// success = false;
	// TestDriver.err("addAllPins()", "should have 2 pins in \"net1\"", "has " +
	// newNetList.getNumberPins(net1));
	// }
	// if (newNetList.getNumberPins(net2) != 2) {
	// success = false;
	// TestDriver.err("addAllPins()", "should have 2 pins in \"net2\"", "has " +
	// newNetList.getNumberPins(net2));
	// }
	// if (newNetList.getNumberPins(net3) != 1) {
	// success = false;
	// TestDriver.err("addAllPins()", "should have 1 pins in \"net3\"", "has " +
	// newNetList.getNumberPins(net3));
	// }
	//
